...
- Captures the remaining characters after `P` from the first string
- If there are no more characters to add → returns `never`
- Otherwise, returns `P` extended by one character
...
2. If `NP` is a valid string and all strings share `NP` → recurse with `NP`
